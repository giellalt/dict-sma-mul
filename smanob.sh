...
echo "and compiles a transducer bin/smanob.fst		 "
echo ""
echo "Usage:"
echo "lookup bin/smanob.fst"
echo "---------------------------------------------------"
echo ""
echo ""

echo "LEXICON Root" > bin/smanob.lexc
cat src/*_smanob.xml | \
grep '^ *<[lt][ >]'  | \
sed 's/^ *//g;'      | \
sed 's/<l /™/g;'     | \
...
tr '>' ':'           | \
tr ' ' '_'           | \
sed 's/$/ # ;/g;'    >> bin/smanob.lexc        

#xfst -e "read lexc < bin/smanob.lexc"

printf "read lexc < bin/smanob.lexc \n\
invert net \n\
save stack bin/smanob.fst \n\
quit \n" > tmpfile
xfst -utf8 < tmpfile
rm -f tmpfile
